...
x_train, x_test, y_train, y_test = train_test_split(
    dataset[features], dataset[label], test_size=0.25, random_state=1
)
...
if not (args.output_training_data is None):
    ...
    training_output_path = os.path.join(
        args.output_training_data, "training_" + args.dataset_name + ".csv"
    )
...
if not (args.output_testing_data is None):
    ...
    testing_output_path = os.path.join(
        args.output_testing_data, "testing_" + args.dataset_name + ".csv"
    )
