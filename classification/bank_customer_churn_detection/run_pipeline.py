...
prepped_data = PipelineData(
    name="preprocessed_data", datastore=default_datastore
).as_dataset()
training_data = PipelineData(
    name="training_data", datastore=default_datastore
).as_dataset()
testing_data = PipelineData(
    name="testing_data", datastore=default_datastore
).as_dataset()
...
features = PipelineParameter(
    name="features", default_value=",".join(features_list)
)
# "gender_Female",
# "gender_Male",
...
step1 = PythonScriptStep(
    ...
    arguments=[
        ...
        dataset_name,
        "--dataset",
        dataset.as_named_input(dataset_name),
        "--preprocessed-data",
        prepped_data,
    ],
)
...
step3 = PythonScriptStep(
    ...
    arguments=["--features", features, "--label", label],
)
...
pipeline = Pipeline(
    ...
    description="Loading data for preprocessing",
)
