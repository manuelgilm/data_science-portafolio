...
from wine_quality_v1.training.mlflow_utils import get_run_id
...
def main():
    ...
    best_params, opt_run_id = optimize(
    ...
    train_metrics = get_classification_metrics(
        y_pred=train_pred, y_true=y_train, prefix="train"
    )
    val_metrics = get_classification_metrics(
        y_pred=val_pred, y_true=y_val, prefix="val"
    )

    with mlflow.start_run(run_id=opt_run_id) as run:
...
def evaluate():
    ...
    config = read_config()
    run_id = get_run_id(config["experiment_name"])
    ...
    metrics = get_classification_metrics(
        y_pred=predictions, y_true=y_test, prefix="test"
    )
